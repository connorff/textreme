...
class AgentMode:
    ...
    def build_conversation_context(
        self, messages: List[Message], contact_name: str = None
    ) -> str:
        if not messages:
    ...
    async def stream_agent_response(
        ...
        try:
            ...
            context = self.build_conversation_context(
                request.conversation_history, request.contact_name
            )
...
Please analyze the conversation and generate {request.max_candidates} candidate messages.""",
            ...
            # Continue conversation until we get candidates
                ...
                for chunk in stream:
                    ...
                    if (
                        ...
                        try:
                            ...
                            if (
                                current_tool_call == "generate_candidates"
                                and "candidates" in tool_result
                            ):
    ...
    def execute_tool(self, tool_name: str, tool_args: dict) -> dict:
        ...
        if tool_name == "analyze_conversation_style":
            ...
            return {"success": True, "candidates": tool_args.get("candidates", [])}
    ...
    async def generate_response(self, request: AgentRequest) -> AgentResponse:
        ...
        async for event in self.stream_agent_response(request):
            if event.type == StreamEventType.CANDIDATE:
                ...
                tool_calls_count = event.metadata.get(
                    "total_tool_calls", tool_calls_count
                )
                reasoning_steps = event.metadata.get(
                    "total_reasoning_steps", reasoning_steps
                )
...
async def main():
    ...
    request = AgentRequest(
        ...
        conversation_history=[
            ...
            Message(id="2", text="Sure, what time works?", date=1, is_from_me=True),
            Message(
                id="3",
                text="How about 2pm?",
                date=2,
